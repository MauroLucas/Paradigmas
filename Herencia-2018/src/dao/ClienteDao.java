...
public class ClienteDao {
	...
	public Cliente traer(String nroCliente)throws HibernateException{
		Cliente objeto = null;
		try {
			iniciaOperacion();
			objeto = (Cliente) session.createQuery("from Cliente c where c.nroCliente like '" + nroCliente + "'").uniqueResult();
		}finally {
			session.close();
		}
		return objeto;
	...
	public PersonaFisica traer(int dni)throws HibernateException{
		...
		try {
			iniciaOperacion();
			objeto = (PersonaFisica) session.createQuery("from PersonaFisica p where p.dni="+dni).uniqueResult();
		}finally {
			session.close();
		}
		return objeto;
